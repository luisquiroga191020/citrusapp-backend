      ...
      const asignaciones = await pool.query(
        `
            SELECT pv.*, p.nombre_completo, p.foto_url, p.codigo, 
                   tn.id as tipo_novedad_id, tn.nombre as tipo_novedad_nombre
            FROM planificacion_visual pv
            JOIN promotores p ON pv.promotor_id = p.id
            LEFT JOIN tipo_novedad tn ON pv.tipo_novedad_id = tn.id
  ...
  async (req, res) => {
    ...
    try {
      ...
      for (const asig of asignaciones) {
        await client.query(
          `
                INSERT INTO planificacion_visual (fecha, zona_id, stand_id, promotor_id, nota, tipo_novedad_id)
                VALUES ($1, $2, $3, $4, $5, $6)
            `,
          [
            fecha,
